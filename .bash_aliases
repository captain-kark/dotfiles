...
function kub-context {
    __k8s_context=$(kub config current-context 2>/dev/null)
    if [ $? -eq 0 ]; then
...
prompt() {
    _exectime=$(perl -MPOSIX -MTime::HiRes=time -e 'printf "%s [%x,%x] (%.3fs)", (strftime "%H:%M:%S%z", localtime), time*1000 >> 12, time, time - @ARGV[0]' $_cmdtime)
    PRE=""
    FMT=""
    POST=""
    inline_status=" "
    ...
    if [ -z $_returncode ]; then
        ...
        if [ -z $PS1_NO_VERBOSE ]; then
            ...
            PRE+="$_returncode_color> $IBlack$_exectime$Color_Off\n"
            PRE+="$IBlack>$IPurple pwd:$_path"
